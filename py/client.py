...
# CONFIG
server = '10.100.61.3'
username='niql2013'#用户名
password='951352'#密码
host_ip = '202.198.175.65'#ip地址
mac = 0x28D24449D70D#mac地址
host_name = '++++++++'#计算机名
...
nic_name = 'enp8s0' #Indicate your nic, e.g. 'eth0.2'.nic_name
bind_ip = '0.0.0.0'
...
def mkpkt(salt, usr, pwd, mac):
    data = b'\x03\x01\x00'+chr(len(usr)+20).encode('ascii')
    data += md5sum(b'\x03\x01'+salt+pwd.encode('ascii'))
    data += usr.ljust(36, '\x00').encode('ascii')
    data += CONTROLCHECKSTATUS.encode('ascii')
    data += ADAPTERNUM.encode('ascii')
    data += dump(int(data[4:10],16)^mac).rjust(6,'\x00') #mac xor md51
